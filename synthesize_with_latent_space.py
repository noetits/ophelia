...
def regression_feat_embed(X, feat_df):
    ...
    return corrs_embed, coeff

def load_features(hp, logdir, conf_path='./tools/opensmile-2.3.0/config/gemaps/eGeMAPSv01a.conf'):
    import glob
    conf_name=conf_path.split('/')[-1].split('.')[0]
    feature_path=os.path.join(logdir,'opensmile_features',conf_name)
    ...
    feat_df.index = data_select.index
    return feat_df

def compute_unsupervised_embeddings(hp, g, model_type):
...
def main_work():
    ...
    if task=='compute_codes':
        ...
        codes, emo_cats=compute_unsupervised_embeddings(hp, g, model_type)
        ...
        #import pdb;pdb.set_trace()
        ice=ICE_TTS_server(hp, embed_reduc, embed, emo_cats, n_polar_axes=8, model_type=model_type)
